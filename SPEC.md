...
4. **Never claim the OSHA rule is law.** It is proposed and unfinalised. Enforcement
   ...
   Clause. That gap is the product's premise — state it accurately.
5. **Forbidden inputs are forbidden.** See `FORBIDDEN_INPUTS` in `constants.py`. No
   age, sex, BMI, fitness, medical history, or home address — ever, anywhere, not
...
"trained" model would be fit to synthetic data from a physics model — circular and
...
Known bias: recent dates are smoother than the archive — diurnal range roughly
...
**Exit — a REGRESSION ANCHOR, not a validation against ground truth.** Reproduces
...
- **The exceedance field is interpolated, not counted** — it can go negative and
